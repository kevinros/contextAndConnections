...
def tf_idf_dots_block(block_start_1, block_start_2, block_size, threshold=0.95):
    matches = {}
...
def tf_idf_dots_blocked(files, block_size, threshold=0.95, fileloc="../matches.txt"):
    matches = {}
...
def tf_idf_dots_blocked_queries(
    corpus, block_size, threshold=0.95, fileloc="matches_queries.txt"
):
    matches = {}
    for i in range(0, len(corpus), block_size):
        for j in range(0, len(corpus), block_size):
            block_match = {}
            mtx = tf_idf_dots_comparitive(
                corpus[i : min(i + block_size, len(corpus))],
                corpus[j : min(j + block_size, len(corpus))],
            )
            coordinates = np.where(mtx > threshold)
            for k in range(len(coordinates[0])):
                x = coordinates[0][k]
...
def main_loop_queries(di, block_size, threshold, fileloc):
    texts = (
        list(
            pd.read_csv(di + "/queries_train.tsv", delimiter="\t", names=["n", "text"])[
                "text"
            ]
        )
        + list(
            pd.read_csv(di + "/queries_val.tsv", delimiter="\t", names=["n", "text"])[
                "text"
            ]
        )
        + list(
            pd.read_csv(di + "/queries_test.tsv", delimiter="\t", names=["n", "text"])[
                "text"
            ]
        )
    )
